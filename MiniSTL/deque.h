#ifndef MINISTL_DEQUE_H_
#define MINISTL_DEQUE_H_
...
//
// 异常保证：
// MiniSTL::deque<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
public:
  // deque 的型别定义
  typedef MiniSTL::allocator<T>                      allocator_type;
  typedef MiniSTL::allocator<T>                      data_allocator;
  typedef MiniSTL::allocator<T*>                     map_allocator;
...
public:
  ...
  template <class IIter, typename std::enable_if<
    MiniSTL::is_input_iterator<IIter>::value, int>::type = 0>
  ...
  deque(deque&& rhs) noexcept
    :begin_(MiniSTL::move(rhs.begin_)),
    end_(MiniSTL::move(rhs.end_)),
    map_(rhs.map_),
...
public:
  ...
  { copy_assign(ilist.begin(), ilist.end(), MiniSTL::forward_iterator_tag{}); }
  ...
  void     push_front(value_type&& value) { emplace_front(MiniSTL::move(value)); }
  void     push_back(value_type&& value)  { emplace_back(MiniSTL::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
  if (this != &rhs)
  {
    ...
    if (len >= rhs.size())
    {
      erase(MiniSTL::copy(rhs.begin_, rhs.end_, begin_), end_);
    }
    else
    {
      iterator mid = rhs.begin() + static_cast<difference_type>(len);
      MiniSTL::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&& ...args)
{
  ...
  {
    data_allocator::construct(begin_.cur - 1, MiniSTL::forward<Args>(args)...);
    --begin_.cur;
  }
  else
  {
    ...
    {
      --begin_;
      data_allocator::construct(begin_.cur, MiniSTL::forward<Args>(args)...);
    }
    catch (...)
    {
      ++begin_;
...
void deque<T>::emplace_back(Args&& ...args)
{
  ...
  {
    data_allocator::construct(end_.cur, MiniSTL::forward<Args>(args)...);
    ++end_.cur;
  }
  else
  {
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
{
  ...
  {
    emplace_front(MiniSTL::forward<Args>(args)...);
    return begin_;
  }
  else if (pos.cur == end_.cur)
  {
    emplace_back(MiniSTL::forward<Args>(args)...);
    return end_ - 1;
  }
  return insert_aux(pos, MiniSTL::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
  ...
  {
    return insert_aux(position, MiniSTL::move(value));
  }
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
  if (position.cur == begin_.cur)
  {
    ...
    MiniSTL::uninitialized_fill_n(new_begin, n, value);
    begin_ = new_begin;
  }
  else if (position.cur == end_.cur)
  {
    ...
    auto new_end = end_ + n;
    MiniSTL::uninitialized_fill_n(end_, n, value);
    end_ = new_end;
  }
  else
  {
...
deque<T>::erase(iterator position)
{
  ...
  {
    MiniSTL::copy_backward(begin_, position, next);
    pop_front();
  }
  else
  {
    MiniSTL::copy(next, end_, position);
    pop_back();
  }
...
deque<T>::erase(iterator first, iterator last)
{
  ...
  else
  {
    ...
    {
      MiniSTL::copy_backward(begin_, first, last);
...
void deque<T>::clear()
{
  ...
  { // 有两个以上的缓冲区
    MiniSTL::destroy(begin_.cur, begin_.last);
    MiniSTL::destroy(end_.first, end_.cur);
  }
  else
  {
    MiniSTL::destroy(begin_.cur, end_.cur);
  }
  shrink_to_fit();
  end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
  if (this != &rhs)
  {
    ...
    MiniSTL::swap(map_, rhs.map_);
    MiniSTL::swap(map_size_, rhs.map_size_);
  }
}
...
map_init(size_type nElem)
{
  ...
  map_size_ = MiniSTL::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
  try
  {
...
fill_init(size_type n, const value_type& value)
{
  ...
  if (n != 0)
  {
    ...
    MiniSTL::uninitialized_fill(end_.first, end_.cur, value);
  }
}
...
copy_init(FIter first, FIter last, forward_iterator_tag)
{
  ...
  MiniSTL::uninitialized_copy(first, last, end_.first);
}
...
{  
  ...
  const size_type len2 = MiniSTL::distance(first, last);
  if (len1 < len2)
  {
    auto next = first;
    MiniSTL::advance(next, len1);
    MiniSTL::copy(first, next, begin_);
    insert_dispatch(end_, next, last, forward_iterator_tag{});
  }
  else
  {
    erase(MiniSTL::copy(first, last, begin_), end_);
  }
}
...
{
  const size_type elems_before = position - begin_;
  value_type value_copy = value_type(MiniSTL::forward<Args>(args)...);
  if (elems_before < (size() / 2))
  { // 在前半段插入
    emplace_front(front());
    auto front1 = begin_;
    ...
    auto pos = position;
    ++pos;
    MiniSTL::copy(front2, pos, front1);
  }
  else
  { // 在后半段插入
    emplace_back(back());
    ...
    --back2;
    position = begin_ + elems_before;
    MiniSTL::copy_backward(position, back2, back1);
  }
  *position = MiniSTL::move(value_copy);
  return position;
}
...
fill_insert(iterator position, size_type n, const value_type& value)
{
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        MiniSTL::uninitialized_copy(begin_, begin_n, new_begin);
        begin_ = new_begin;
        MiniSTL::copy(begin_n, position, old_begin);
        MiniSTL::fill(position - n, position, value_copy);
      }
      else
      {
        MiniSTL::uninitialized_fill(
          MiniSTL::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
        begin_ = new_begin;
        MiniSTL::fill(old_begin, position, value_copy);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      ...
      {
        auto end_n = end_ - n;
        MiniSTL::uninitialized_copy(end_n, end_, end_);
        end_ = new_end;
        MiniSTL::copy_backward(position, end_n, old_end);
        MiniSTL::fill(position, position + n, value_copy);
      }
      else
      {
        MiniSTL::uninitialized_fill(end_, position + n, value_copy);
        MiniSTL::uninitialized_copy(position, end_, position + n);
        end_ = new_end;
        MiniSTL::fill(position, old_end, value_copy);
      }
    }
    catch (...)
    {
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        begin_ = new_begin;
        MiniSTL::copy(begin_n, position, old_begin);
        MiniSTL::copy(first, last, position - n);
      }
      else
      {
        auto mid = first;
        MiniSTL::advance(mid, n - elems_before);
        MiniSTL::uninitialized_copy(first, mid,
                                  MiniSTL::uninitialized_copy(begin_, position, new_begin));
        begin_ = new_begin;
        MiniSTL::copy(mid, last, old_begin);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      if (elems_after > n)
      {
        ...
        end_ = new_end;
        MiniSTL::copy_backward(position, end_n, old_end);
        MiniSTL::copy(first, last, position);
      }
      else
      {
        auto mid = first;
        MiniSTL::advance(mid, elems_after);
        MiniSTL::uninitialized_copy(position, end_,
                                  MiniSTL::uninitialized_copy(mid, last, end_));
        end_ = new_end;
...
insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
  ...
  if (position.cur == begin_.cur)
  {
    ...
    try
    {
      MiniSTL::uninitialized_copy(first, last, new_begin);
      begin_ = new_begin;
    }
    catch (...)
    {
  ...
  else if (position.cur == end_.cur)
  {
    ...
    try
    {
      MiniSTL::uninitialized_copy(first, last, end_);
      end_ = new_end;
    }
    catch (...)
    {
...
{
  const size_type new_map_size = MiniSTL::max(map_size_ << 1,
