#ifndef MINISTL_HASHTABLE_H_
#define MINISTL_HASHTABLE_H_
...
struct hashtable_node
{
  ...
  hashtable_node(hashtable_node&& node) :next(node.next), value(MiniSTL::move(node.value))
  {
    node.next = nullptr;
  }
};
...
struct ht_iterator_base :public MiniSTL::iterator<MiniSTL::forward_iterator_tag, T>
{
  typedef MiniSTL::hashtable<T, Hash, KeyEqual>         hashtable;
  typedef ht_iterator_base<T, Hash, KeyEqual>         base;
  typedef MiniSTL::ht_iterator<T, Hash, KeyEqual>       iterator;
  typedef MiniSTL::ht_const_iterator<T, Hash, KeyEqual> const_iterator;
...
// local iterator
template <class T>
struct ht_local_iterator :public MiniSTL::iterator<MiniSTL::forward_iterator_tag, T>
{
  typedef T                          value_type;
  typedef value_type*                pointer;
  typedef value_type&                reference;
...
template <class T>
struct ht_const_local_iterator :public MiniSTL::iterator<MiniSTL::forward_iterator_tag, T>
{
  typedef T                          value_type;
  typedef const value_type*          pointer;
  typedef const value_type&          reference;
...
inline size_t ht_next_prime(size_t n)
{
  ...
  const size_t* pos = MiniSTL::lower_bound(first, last, n);
...
{  
  ...
  friend struct MiniSTL::ht_iterator<T, Hash, KeyEqual>;
  friend struct MiniSTL::ht_const_iterator<T, Hash, KeyEqual>;
...
public:
  ...
  typedef MiniSTL::vector<node_ptr>                     bucket_type;

  typedef MiniSTL::allocator<T>                         allocator_type;
  typedef MiniSTL::allocator<T>                         data_allocator;
  typedef MiniSTL::allocator<node_type>                 node_allocator;
  ...
  typedef MiniSTL::ht_iterator<T, Hash, KeyEqual>       iterator;
  typedef MiniSTL::ht_const_iterator<T, Hash, KeyEqual> const_iterator;
  typedef MiniSTL::ht_local_iterator<T>                 local_iterator;
  typedef MiniSTL::ht_const_local_iterator<T>           const_local_iterator;
...
public:
  ...
  template <class Iter, typename std::enable_if<
    ...
    hashtable(Iter first, Iter last,
              ...
              const Hash& hash = Hash(),
              const KeyEqual& equal = KeyEqual())
    :size_(MiniSTL::distance(first, last)), mlf_(1.0f), hash_(hash), equal_(equal)
  {
    init(MiniSTL::max(bucket_count, static_cast<size_type>(MiniSTL::distance(first, last))));
  }
  ...
  hashtable(hashtable&& rhs) noexcept
    ...
    equal_(rhs.equal_)
  {
    buckets_ = MiniSTL::move(rhs.buckets_);
    rhs.bucket_size_ = 0;
    rhs.size_ = 0;
    rhs.mlf_ = 0.0f;
  }
  ...
  { return emplace_multi(MiniSTL::forward<Args>(args)...); }

  template <class ...Args>
  iterator emplace_unique_use_hint(const_iterator /*hint*/, Args&& ...args)
  { return emplace_unique(MiniSTL::forward<Args>(args)...).first; }
  ...
  { return emplace_multi(MiniSTL::move(value)); }
  ...
  { return emplace_unique(MiniSTL::move(value)); }
...
{
  hashtable tmp(MiniSTL::move(rhs));
  swap(tmp);
  return *this;
}
...
{
  auto np = create_node(MiniSTL::forward<Args>(args)...);
  try
  {
...
insert_unique_noresize(const value_type& value)
{
  ...
  for (auto cur = first; cur; cur = cur->next)
  {
    if (is_equal(value_traits::get_key(cur->value), value_traits::get_key(value)))
      return MiniSTL::make_pair(iterator(cur, this), false);
  }
  // 让新节点成为链表的第一个节点
  ...
  buckets_[n] = tmp;
  ++size_;
  return MiniSTL::make_pair(iterator(tmp, this), true);
}
  ...
  for (node_ptr first = buckets_[n]; first; first = first->next)
  {
    ...
    { // 如果出现相等的键值
      ...
      { // 整个链表都相等，查找下一个链表出现的位置
        if (buckets_[m])
          return MiniSTL::make_pair(iterator(first, this), iterator(buckets_[m], this));
      }
...
swap(hashtable& rhs) noexcept
{
  if (this != &rhs)
  {
    ...
    MiniSTL::swap(bucket_size_, rhs.bucket_size_);
    MiniSTL::swap(size_, rhs.size_);
    MiniSTL::swap(mlf_, rhs.mlf_);
    MiniSTL::swap(hash_, rhs.hash_);
    MiniSTL::swap(equal_, rhs.equal_);
  }
}
...
create_node(Args&& ...args)
{
  ...
  try
  {
    data_allocator::construct(MiniSTL::address_of(tmp->value), MiniSTL::forward<Args>(args)...);
    tmp->next = nullptr;
  }
  catch (...)
  {
...
{
  data_allocator::destroy(MiniSTL::address_of(node->value));
...
{
  rehash_if_need(MiniSTL::distance(first, last));
...
insert_node_unique(node_ptr np)
{
  ...
  if (cur == nullptr)
  {
    buckets_[n] = np;
    ++size_;
    return MiniSTL::make_pair(iterator(np, this), true);
  }
  for (; cur; cur = cur->next)
  {
    if (is_equal(value_traits::get_key(cur->value), value_traits::get_key(np->value)))
...
bool hashtable<T, Hash, KeyEqual>::equal_to_multi(const hashtable& other)
{
  ...
  for (auto f = begin(), l = end(); f != l;)
  {
    ...
    if (MiniSTL::distance(p1.first, p1.last) != MiniSTL::distance(p2.first, p2.last) ||
        !MiniSTL::is_permutation(p1.first, p2.last, p2.first, p2.last))
      return false;
    f = p1.last;
  }
  return true;
