#ifndef MINISTL_LIST_H_
#define MINISTL_LIST_H_
...
//
// 异常保证：
// MiniSTL::list<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
// list 的迭代器设计
template <class T>
struct list_iterator : public MiniSTL::iterator<MiniSTL::bidirectional_iterator_tag, T>
{
...
public:
  ...
  typedef MiniSTL::allocator<list_node_base<T>>      base_allocator;
  typedef MiniSTL::allocator<list_node<T>>           node_allocator;
...
public:
  ...
  void     emplace_front(Args&& ...args)
  {
    ...
    auto link_node = create_node(MiniSTL::forward<Args>(args)...);
  ...
  {
    THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
    auto link_node = create_node(MiniSTL::move(value));
    ++size_;
    return link_iter_node(pos, link_node->as_base());
  }
  ...
  void unique()
  { unique(MiniSTL::equal_to<T>()); }
  template <class BinaryPredicate>
  void unique(BinaryPredicate pred);

  void merge(list& x)
  { merge(x, MiniSTL::less<T>()); }
  template <class Compare>
  void merge(list& x, Compare comp);

  void sort()
  { list_sort(begin(), end(), size(), MiniSTL::less<T>()); }
...
void list<T>::reverse()
{
  ...
  {
    MiniSTL::swap(i.node_->prev, i.node_->next);
    i.node_ = i.node_->prev;
  }
  MiniSTL::swap(e.node_->prev, e.node_->next);
}
...
list<T>::create_node(Args&& ...args)
{
  ...
  try
  {
    data_allocator::construct(MiniSTL::address_of(p->value), MiniSTL::forward<Args>(args)...);
    p->prev = nullptr;
    p->next = nullptr;
  }
  catch (...)
...
{
  data_allocator::destroy(MiniSTL::address_of(p->value));
  node_allocator::deallocate(p);
}
