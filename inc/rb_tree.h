#ifndef MINISTL_RB_TREE_H_
#define MINISTL_RB_TREE_H_
...
template <class T>
struct rb_tree_iterator_base :public MiniSTL::iterator<MiniSTL::bidirectional_iterator_tag, T>
{
...
public:
  ...
  typedef MiniSTL::allocator<base_type>              base_allocator;
...
public:
  ...
  template <class ...Args>
  MiniSTL::pair<iterator, bool> emplace_unique(Args&& ...args);
  ...
  {
    return emplace_multi_use_hint(hint, MiniSTL::move(value));
  }
  ...
  MiniSTL::pair<iterator, bool> insert_unique(const value_type& value);
  MiniSTL::pair<iterator, bool> insert_unique(value_type&& value)
  {
  ...
  {
    return emplace_unique_use_hint(hint, MiniSTL::move(value));
  }
  ...
  size_type      count_multi(const key_type& key) const
  {
    ...
    return static_cast<size_type>(MiniSTL::distance(p.first, p.second));
  }
...
private:
  ...
  // get insert pos
  MiniSTL::pair<base_ptr, bool> 
           get_insert_multi_pos(const key_type& key);
  MiniSTL::pair<MiniSTL::pair<base_ptr, bool>, bool> 
...
rb_tree<T, Compare>::
rb_tree(rb_tree&& rhs) noexcept
  :header_(MiniSTL::move(rhs.header_)),
  node_count_(rhs.node_count_),
  key_comp_(rhs.key_comp_)
{
  rhs.reset();
...
emplace_multi(Args&& ...args)
{
  ...
  node_ptr np = create_node(MiniSTL::forward<Args>(args)...);
...
template <class T, class Compare>
template <class ...Args>
MiniSTL::pair<typename rb_tree<T, Compare>::iterator, bool> 
rb_tree<T, Compare>::
emplace_unique(Args&& ...args)
{
  ...
  if (res.second)
  { // 插入成功
    return MiniSTL::make_pair(insert_node_at(res.first.first, np, res.first.second), true);
  }
  destroy_node(np);
  return MiniSTL::make_pair(iterator(res.first.first), false);
}
...
insert_unique(const value_type& value)
{
  ...
  if (res.second)
  { // 插入成功
    return MiniSTL::make_pair(insert_value_at(res.first.first, value, res.first.second), true);
  }
  return MiniSTL::make_pair(res.first.first, false);
}
...
swap(rb_tree& rhs) noexcept
{
  if (this != &rhs)
  {
    MiniSTL::swap(header_, rhs.header_);
    MiniSTL::swap(node_count_, rhs.node_count_);
    MiniSTL::swap(key_comp_, rhs.key_comp_);
  }
}
...
template <class T, class Compare>
MiniSTL::pair<typename rb_tree<T, Compare>::base_ptr, bool>
rb_tree<T, Compare>::get_insert_multi_pos(const key_type& key)
{
  ...
  return MiniSTL::make_pair(y, add_to_left);
}

// get_insert_unique_pos 函数
template <class T, class Compare>
MiniSTL::pair<MiniSTL::pair<typename rb_tree<T, Compare>::base_ptr, bool>, bool>
...
{ // 返回一个 pair，第一个值为一个 pair，包含插入点的父节点和一个 bool 表示是否在左边插入，
  ...
  if (add_to_left)
  {
    ...
    { // 如果树为空树或插入点在最左节点处，肯定可以插入新的节点
      return MiniSTL::make_pair(MiniSTL::make_pair(y, true), true);
    }
    else
  ...
  { // 表明新节点没有重复
    return MiniSTL::make_pair(MiniSTL::make_pair(y, add_to_left), true);
  }
  // 进行至此，表示新节点与现有节点键值重复
  return MiniSTL::make_pair(MiniSTL::make_pair(y, add_to_left), false);
}
