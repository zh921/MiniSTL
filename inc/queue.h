#ifndef MINISTL_QUEUE_H_
#define MINISTL_QUEUE_H_
...
// 模板类 queue
// 参数一代表数据类型，参数二代表底层容器类型，缺省使用 MiniSTL::deque 作为底层容器
...
public:
  ...
  { c_.emplace_back(MiniSTL::forward<Args>(args)...); }

  void push(const value_type& value) 
  { c_.push_back(value); }
  void push(value_type&& value)      
  { c_.emplace_back(MiniSTL::move(value)); }
  ...
  void swap(queue& rhs) noexcept(noexcept(MiniSTL::swap(c_, rhs.c_)))
  { MiniSTL::swap(c_, rhs.c_); }
...
// 模板类 priority_queue
// 参数一代表数据类型，参数二代表容器类型，缺省使用 MiniSTL::vector 作为底层容器
// 参数三代表比较权值的方式，缺省使用 MiniSTL::less 作为比较方式
template <class T, class Container = MiniSTL::vector<T>,
  class Compare = MiniSTL::less<typename Container::value_type>>
...
public:
  ...
  explicit priority_queue(size_type n)
    :c_(n)
  {
    MiniSTL::make_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  priority_queue(priority_queue&& rhs) 
    :c_(MiniSTL::move(rhs.c_)), comp_(rhs.comp_)
  {
    MiniSTL::make_heap(c_.begin(), c_.end(), comp_);
  }
...
public:
  ...
  {
    c_.emplace_back(MiniSTL::forward<Args>(args)...);
    MiniSTL::push_heap(c_.begin(), c_.end(), comp_);
  }
  ...
  void push(value_type&& value)
  {
    c_.push_back(MiniSTL::move(value));
    MiniSTL::push_heap(c_.begin(), c_.end(), comp_);
  }

  void pop()
  {
    MiniSTL::pop_heap(c_.begin(), c_.end(), comp_);
    c_.pop_back();
  }
  ...
  void swap(priority_queue& rhs) noexcept(noexcept(MiniSTL::swap(c_, rhs.c_)) &&
                                          noexcept(MiniSTL::swap(comp_, rhs.comp_)))
  {
    MiniSTL::swap(c_, rhs.c_);
    MiniSTL::swap(comp_, rhs.comp_);
  }
