#ifndef MINISTL_ALGO_H_
#define MINISTL_ALGO_H_
...
search_n(ForwardIter first, ForwardIter last, Size n, const T& value)
{
  ...
  {
    first = MiniSTL::find(first, last, value);
    while (first != last)
    {
      auto m = n - 1;
      auto i = first;
      ...
      {
        first = MiniSTL::find(i, last, value);
      }
    }
    return last;
  }
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag)
{
  ...
  reviter1 rresult = MiniSTL::search(reviter1(last1), rlast1, reviter2(last2), rlast2);
  if (rresult == rlast1)
  {
    return last1;
  }
  else
  {
    ...
    MiniSTL::advance(result, -MiniSTL::distance(first2, last2));
    return result;
  }
}
         ...
         ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  return MiniSTL::find_end_dispatch(first1, last1, first2, last2, Category1(), Category2());
}
                  ...
                  forward_iterator_tag, forward_iterator_tag, Compared comp)
{
  ...
  else
  {
    ...
    {
      // 利用 search 查找某个子序列的首次出现点，找不到则返回 last1
      auto new_result = MiniSTL::search(first1, last1, first2, last2, comp);
      if (new_result == last1)
      {
        return result;
      }
                  ...
                  bidirectional_iterator_tag, bidirectional_iterator_tag, Compared comp)
{
  ...
  reviter1 rlast1(first1);
  reviter2 rlast2(first2);
  reviter1 rresult = MiniSTL::search(reviter1(last1), rlast1, reviter2(last2), rlast2, comp);
  if (rresult == rlast1)
  {
    return last1;
  }
         ...
         ForwardIter2 first2, ForwardIter2 last2, Compared comp)
{
  typedef typename iterator_traits<ForwardIter1>::iterator_category Category1;
  typedef typename iterator_traits<ForwardIter2>::iterator_category Category2;
  return MiniSTL::find_end_dispatch(first1, last1, first2, last2, Category1(), Category2(), comp);
}
...
{
  return MiniSTL::lbound_dispatch(first, last, value, iterator_category(first));
}
...
{
  return MiniSTL::lbound_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  return MiniSTL::ubound_dispatch(first, last, value, iterator_category(first));
}
...
{
  return MiniSTL::ubound_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  auto i = MiniSTL::lower_bound(first, last, value);
...
template <class ForwardIter, class T>
MiniSTL::pair<ForwardIter, ForwardIter>
                ...
                const T& value, forward_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = MiniSTL::lower_bound(first, last, value);
      MiniSTL::advance(first, len);
      right = MiniSTL::upper_bound(++middle, first, value);
      return MiniSTL::pair<ForwardIter, ForwardIter>(left, right);
    }
  }
  return MiniSTL::pair<ForwardIter, ForwardIter>(last, last);
}

// erange_dispatch 的 random_access_iterator_tag 版本
template <class RandomIter, class T>
MiniSTL::pair<RandomIter, RandomIter>
                ...
                const T& value, random_access_iterator_tag)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = MiniSTL::lower_bound(first, middle, value);
      right = MiniSTL::upper_bound(++middle, first + len, value);
      return MiniSTL::pair<RandomIter, RandomIter>(left, right);
    }
  }
  return MiniSTL::pair<RandomIter, RandomIter>(last, last);
}
...
{
  return MiniSTL::erange_dispatch(first, last, value, iterator_category(first));
}
                ...
                const T& value, forward_iterator_tag, Compared comp)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = MiniSTL::lower_bound(first, last, value, comp);
      MiniSTL::advance(first, len);
      right = MiniSTL::upper_bound(++middle, first, value, comp);
      return MiniSTL::pair<ForwardIter, ForwardIter>(left, right);
    }
  }
  return MiniSTL::pair<ForwardIter, ForwardIter>(last, last);
                ...
                const T& value, random_access_iterator_tag, Compared comp)
{
  ...
  while (len > 0)
  {
    ...
    {
      left = MiniSTL::lower_bound(first, middle, value, comp);
      right = MiniSTL::upper_bound(++middle, first + len, value, comp);
      return MiniSTL::pair<RandomIter, RandomIter>(left, right);
    }
  }
  return MiniSTL::pair<RandomIter, RandomIter>(last, last);
...
{
  return MiniSTL::erange_dispatch(first, last, value, iterator_category(first), comp);
}
...
{
  first = MiniSTL::find(first, last, value);  // 利用 find 找出第一个匹配的地方
  auto next = first;
...
{
  first = MiniSTL::find_if(first, last, unary_pred);  // 利用 find_if 找出第一个匹配的地方
  auto next = first;
  return first == last ? first : MiniSTL::remove_copy_if(++next, last, first, unary_pred);
}
...
{
  MiniSTL::reverse_dispatch(first, last, iterator_category(first));
}
...
void random_shuffle(RandomIter first, RandomIter last)
{
  ...
  {
    MiniSTL::iter_swap(i, first + (rand() % (i - first + 1)));
  }
}
                    ...
                    RandomNumberGenerator& rand)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    MiniSTL::iter_swap(i, first + (rand(i - first + 1) % len));
  }
}
...
{
  MiniSTL::reverse_dispatch(first, middle, bidirectional_iterator_tag());
  MiniSTL::reverse_dispatch(middle, last, bidirectional_iterator_tag());
                ...
                RandomIter last, random_access_iterator_tag)
{
  ...
  if (l == r)
  {
    MiniSTL::swap_ranges(first, middle, middle);
    return result;
  }
...
rotate(ForwardIter first, ForwardIter middle, ForwardIter last)
{
  ...
  if (middle == last)
    return first;
  return MiniSTL::rotate_dispatch(first, middle, last, iterator_category(first));
}
...
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
                        ...
                        BinaryPred pred)
{
  constexpr bool is_ra_it = MiniSTL::is_random_access_iterator<ForwardIter1>::value
    && MiniSTL::is_random_access_iterator<ForwardIter2>::value;
  if (is_ra_it)
  {
  ...
  {
    auto len1 = MiniSTL::distance(first1, last1);
    auto len2 = MiniSTL::distance(first2, last2);
    if (len1 == 0 && len2 == 0)
      return true;
    if (len1 != len2)
      return false;
                    ...
                    ForwardIter2 first2, ForwardIter2 last2)
{
  ...
  static_assert(std::is_same<v1, v2>::value,
                "the type should be same in MiniSTL::is_permutation");
...
bool next_permutation(BidirectionalIter first, BidirectionalIter last)
{
  ...
  for (;;)
  {
    ...
    {                 // 找到第一对小于关系的元素
      auto j = last;
      while (!(*i < *--j)) {}
      MiniSTL::iter_swap(i, j);       // 交换 i，j 所指元素
      MiniSTL::reverse(ii, last);     // 将 ii 之后的所有元素反转
      return true;
    }
    if (i == first)
    {
                          ...
                          BidirectionalIter last, Distance len1, Distance len2)
{
  ...
  auto new_middle = MiniSTL::rotate(first_cut, middle, second_cut);
  MiniSTL::merge_without_buffer(first, first_cut, new_middle, len11, len22);
  MiniSTL::merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22);
}
                ...
                BidirectionalIter2 buffer, Distance buffer_size)
{
  ...
  {
    return MiniSTL::rotate(first, middle, last);
  }
}
                    ...
                    Pointer buffer, Distance buffer_size)
{
  ...
  {
    Pointer buffer_end = MiniSTL::copy(first, middle, buffer);
    MiniSTL::merge(buffer, buffer_end, middle, last, first);
  }
  else if (len2 <= buffer_size)
  {
    Pointer buffer_end = MiniSTL::copy(middle, last, buffer);
    MiniSTL::merge_backward(first, middle, buffer, buffer_end, last);
  }
  else
  {  // 区间长度太长，分割递归处理
    auto first_cut = first;
    ...
    auto new_middle = MiniSTL::rotate_adaptive(first_cut, middle, second_cut,
                                             len1 - len11, len22, buffer, buffer_size);
    MiniSTL::merge_adaptive(first, first_cut, new_middle, len11, len22, buffer, buffer_size);
    MiniSTL::merge_adaptive(new_middle, second_cut, last, len1 - len11,
                  ...
                  BidirectionalIter last, T*)
{
  ...
  if (!buf.begin())
  {
    MiniSTL::merge_without_buffer(first, middle, last, len1, len2);
  }
  else
  {
    MiniSTL::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size());
  }
}
              ...
              BidirectionalIter last)
{
  if (first == middle || middle == last)
    return;
  MiniSTL::inplace_merge_aux(first, middle, last, value_type(first));
}
                          ...
                          Compared comp)
{
  ...
  if (len1 > len2)
  {
    len11 = len1 >> 1;
    MiniSTL::advance(first_cut, len11);
    second_cut = MiniSTL::lower_bound(middle, last, *first_cut, comp);
    len22 = MiniSTL::distance(middle, second_cut);
  }
  else
  {
    len22 = len2 >> 1;
    MiniSTL::advance(second_cut, len22);
    first_cut = MiniSTL::upper_bound(first, middle, *second_cut, comp);
    len11 = MiniSTL::distance(first, first_cut);
  }
  auto new_middle = MiniSTL::rotate(first_cut, middle, second_cut);
  MiniSTL::merge_without_buffer(first, first_cut, new_middle, len11, len22, comp);
  MiniSTL::merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22, comp);
}
                    ...
                    Pointer buffer, Distance buffer_size, Compared comp)
{
  ...
  {
    Pointer buffer_end = MiniSTL::copy(first, middle, buffer);
    MiniSTL::merge(buffer, buffer_end, middle, last, first, comp);
  }
  else if (len2 <= buffer_size)
  {
    Pointer buffer_end = MiniSTL::copy(middle, last, buffer);
    MiniSTL::merge_backward(first, middle, buffer, buffer_end, last, comp);
  }
  else
  {  // 区间长度太长，分割递归处理
    auto first_cut = first;
    ...
    auto new_middle = MiniSTL::rotate_adaptive(first_cut, middle, second_cut, len1 - len11,
                                             len22, buffer, buffer_size);
    MiniSTL::merge_adaptive(first, first_cut, new_middle, len11,
                  ...
                  BidirectionalIter last, T*, Compared comp)
{
  ...
  if (!buf.begin())
  {
    MiniSTL::merge_without_buffer(first, middle, last, len1, len2, comp);
  }
  else
  {
    MiniSTL::merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size(), comp);
  }
}
              ...
              BidirectionalIter last, Compared comp)
{
  if (first == middle || middle == last)
    return;
  MiniSTL::inplace_merge_aux(first, middle, last, value_type(first), comp);
}
                  ...
                  RandomIter last)
{
  ...
  for (auto i = middle; i < last; ++i)
  {
    if (*i < *first)
    {
      MiniSTL::pop_heap_aux(first, middle, i, *i, distance_type(first));
    }
  }
                  ...
                  RandomIter last, Compared comp)
{
  ...
  for (auto i = middle; i < last; ++i)
  {
    if (comp(*i, *first))
    {
      MiniSTL::pop_heap_aux(first, middle, i, *i, distance_type(first), comp);
    }
  }
  MiniSTL::sort_heap(first, middle, comp);
}
               ...
               Distance*)
{
  ...
  MiniSTL::make_heap(result_first, result_iter);
  while (first != last)
  {
    if (*first < *result_first)
    {
      MiniSTL::adjust_heap(result_first, static_cast<Distance>(0),
                           result_iter - result_first, *first);
    }
    ++first;
  }
  MiniSTL::sort_heap(result_first, result_iter);
  return result_iter;
}
...
partial_sort_copy(InputIter first, InputIter last,
                  RandomIter result_first, RandomIter result_last)
{
  return MiniSTL::psort_copy_aux(first, last, result_first, result_last,
               ...
               Distance*, Compared comp)
{
  ...
  MiniSTL::make_heap(result_first, result_iter, comp);
  while (first != last)
  {
  ...
  MiniSTL::sort_heap(result_first, result_iter, comp);
  return result_iter;
}
...
// 其余放到 result_false 的输出区间，并返回一个 MiniSTL::pair 指向这两个区间的尾部
/*****************************************************************************************/
template <class InputIter, class OutputIter1, class OutputIter2, class UnaryPredicate>
MiniSTL::pair<OutputIter1, OutputIter2>
               ...
               UnaryPredicate unary_pred)
{
  ...
  return MiniSTL::pair<OutputIter1, OutputIter2>(result_true, result_false);
}
...
void intro_sort(RandomIter first, RandomIter last, Size depth_limit)
{
  while (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    if (depth_limit == 0)
    {                      // 到达最大分割深度限制
      MiniSTL::partial_sort(first, last, last);  // 改用 heap_sort
      return;
    }
    --depth_limit;
    auto mid = MiniSTL::median(*(first), *(first + (last - first) / 2), *(last - 1));
    auto cut = MiniSTL::unchecked_partition(first, last, mid);
    MiniSTL::intro_sort(cut, last, depth_limit);
    last = cut;
  }
}
...
void unchecked_insertion_sort(RandomIter first, RandomIter last)
{
  ...
  {
    MiniSTL::unchecked_linear_insert(i, *i);
  }
}
...
void insertion_sort(RandomIter first, RandomIter last)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    ...
    if (value < *first)
    {
      MiniSTL::copy_backward(first, i, i + 1);
      *first = value;
    }
    else
    {
      MiniSTL::unchecked_linear_insert(i, value);
    }
  }
}
...
void final_insertion_sort(RandomIter first, RandomIter last)
{
  ...
  {
    MiniSTL::insertion_sort(first, first + kSmallSectionSize);
    MiniSTL::unchecked_insertion_sort(first + kSmallSectionSize, last);
  }
  else
  {
    MiniSTL::insertion_sort(first, last);
  }
}
...
void sort(RandomIter first, RandomIter last)
{
  if (first != last)
  {
    ...
    MiniSTL::intro_sort(first, last, slg2(last - first) * 2);
    MiniSTL::final_insertion_sort(first, last);
  }
}
                ...
                Size depth_limit, Compared comp)
{
  while (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    if (depth_limit == 0)
    {                            // 到达最大分割深度限制
      MiniSTL::partial_sort(first, last, last, comp);  // 改用 heap_sort
      return;
    }
    --depth_limit;
    auto mid = MiniSTL::median(*(first), *(first + (last - first) / 2), *(last - 1));
    auto cut = MiniSTL::unchecked_partition(first, last, mid, comp);
    MiniSTL::intro_sort(cut, last, depth_limit, comp);
    last = cut;
  }
}
                              ...
                              Compared comp)
{
  for (auto i = first; i != last; ++i)
  {
    MiniSTL::unchecked_linear_insert(i, *i, comp);
  }
}
...
void insertion_sort(RandomIter first, RandomIter last, Compared comp)
{
  ...
  for (auto i = first + 1; i != last; ++i)
  {
    ...
    {
      MiniSTL::unchecked_linear_insert(i, value, comp);
    }
  }
}
...
void final_insertion_sort(RandomIter first, RandomIter last, Compared comp)
{
  if (static_cast<size_t>(last - first) > kSmallSectionSize)
  {
    MiniSTL::insertion_sort(first, first + kSmallSectionSize, comp);
    MiniSTL::unchecked_insertion_sort(first + kSmallSectionSize, last, comp);
  }
  else
  {
    MiniSTL::insertion_sort(first, last, comp);
  }
}
...
void sort(RandomIter first, RandomIter last, Compared comp)
{
  ...
  {
    // 内省式排序，将区间分为一个个小区间，然后对整体进行插入排序
    MiniSTL::intro_sort(first, last, slg2(last - first) * 2, comp);
    MiniSTL::final_insertion_sort(first, last, comp);
  }
}
                 ...
                 RandomIter last)
{
  ...
  while (last - first > 3)
  {
    auto cut = MiniSTL::unchecked_partition(first, last, MiniSTL::median(*first,
										  *(first + (last - first) / 2),
										  *(last - 1)));
...
unique_copy(InputIter first, InputIter last, OutputIter result)
{
  if (first == last)
    return result;
  return MiniSTL::unique_copy_dispatch(first, last, result, iterator_category(result));
}
...
unique_copy(InputIter first, InputIter last, OutputIter result, Compared comp)
{
  if (first == last)
    return result;
  return MiniSTL::unique_copy_dispatch(first, last, result, iterator_category(result), comp);
}
...
{
  first = MiniSTL::adjacent_find(first, last);
...
{
  first = MiniSTL::adjacent_find(first, last, comp);
  return MiniSTL::unique_copy(first, last, first, comp);
}
