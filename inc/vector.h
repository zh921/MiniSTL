#ifndef MINISTL_VECTOR_H_
#define MINISTL_VECTOR_H_
...
//
// 异常保证：
// MiniSTL::vecotr<T> 满足基本异常保证，部分函数无异常保证，并对以下函数做强异常安全保证：
//   * emplace
//   * emplace_back
//   * push_back
...
class vector
{
  static_assert(!std::is_same<bool, T>::value, "vector<bool> is abandoned in MiniSTL");
public:
  // vector 的嵌套型别定义
...
vector<T>& vector<T>::operator=(const vector& rhs)
{
  if (this != &rhs)
  {
    ...
    {
      auto i = MiniSTL::copy(rhs.begin(), rhs.end(), begin());
      data_allocator::destroy(i, end_);
      end_ = begin_ + len;
    }
    else
    { 
      MiniSTL::copy(rhs.begin(), rhs.begin() + size(), begin_);
      MiniSTL::uninitialized_copy(rhs.begin() + size(), rhs.end(), end_);
...
vector<T>::emplace(const_iterator pos, Args&& ...args)
{
  ...
  else if (end_ != cap_)
  {
    ...
    *xpos = value_type(MiniSTL::forward<Args>(args)...);
  }
  else
  {
    reallocate_emplace(xpos, MiniSTL::forward<Args>(args)...);
  }
  return begin() + n;
}
